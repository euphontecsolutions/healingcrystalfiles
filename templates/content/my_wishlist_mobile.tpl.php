  ...
  while($wishlist = tep_db_fetch_array($wishlist_query_raw)) {
	...
	while($wishlist_products_attributes = tep_db_fetch_array($wishlist_products_attributes_query))
	{
	  ...
	  $products_name = '<table valign="top" border="0" cellspacing="2" cellpadding="2" >' .
                       ...
                       '    <td class="productListing-data" valign="top"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_detail['products_id']) . '"><b class="name">' . $products_detail['products_name'] . '</b></a>';
	  ...
	  $products_name .= '<br><br><small><i class="name1"> - ' . $products_options['products_options_name'] . ' ' . $products_attributes["products_attributes_name"] . '</i></small>';
