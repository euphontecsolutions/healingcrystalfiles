...
@media screen and (max-width: 600px) {
	...
	table#cart tbody td { display: block; padding: .6rem; min-width:120px;}
...
?>
    ...
    if(empty($products)) {
        ?>
        <span style="font:caption">No item in cart</span>
        <?php
    }
    else {
...
$cart_query = tep_db_query("SELECT * FROM `customers_basket` WHERE products_id='".$product_cart_id."'");
       ...
       $price_added = $cart_array['app_added_price'];
              if($products[$i]['final_price']!=$price_added && $price_added!='') {           
                  $products_name .='Note:Price at the time of checkout was $'.$price_added;
          }
        }
      }
    ...
    margin-left: 12px;">' . $currencies->display_price($products[$i]['final_price'], tep_get_tax_rate($products[$i]['tax_class_id'])) . '</b>';
                                              '<div class="col-md-12"><div><div style="width: 20%; float: left;">'.tep_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="1" maxlength="4" style="width: 60%;"') . tep_draw_hidden_field('products_id[]', $products[$i]['id']).'</div><div  align="left">'. tep_template_image_submit('button_update_cart.gif', IMAGE_BUTTON_UPDATE_CART, 'onClick="document.forms[\'cart_quantity\'].submit();" onmouseover="Tip(\'Update Quantity\')" onmouseout="UnTip()"').'</div></div><div><div  align="left"><a href="'.$_SERVER['PHP_SELF'] . '?action=add_to_wishlist&products_id='.$products[$i]['id'].'&qty='.$products[$i]['quantity'].'" onmouseover="Tip(\'Move this item from your Shopping Cart to your Wish List.\')" onmouseout="UnTip()">'.tep_template_image_button('button_save_later.gif', IMAGE_BUTTON_SAVE_LATER).'</a></div></div><div><div align="left" ><a href="'.$_SERVER['PHP_SELF'] . '?action=remove_product&products_id='.$products[$i]['id'].'" onmouseover="Tip(\'Remove item\')" onmouseout="UnTip()">'.tep_template_image_button('button_remove.gif', IMAGE_BUTTON_REMOVE).'</a></div></div></div><hr class="style-eight" />';
	...
	<table id="cart" class="table table-hover table-condensed">
							...
							<td>
								<div class="row">
									...
									<div class="col-sm-10">
										...
										<p style="color:#777;"><h4><?php echo $products_name;?></h4></p>
									</div>
								</div>
							</td>
							<?php 
	  ...
	  ?>
							...
							<td data-th="Quantity" style="background: white;">
								<?php echo tep_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="1" maxlength="4" style="width: 13%;"'). tep_draw_hidden_field('products_id[]', $products[$i]['id']);?>
							
								<button class="btn btn-info btn-sm" onclick="document.forms['cart_quantity'].submit();" onmouseover="Tip('Update Quantity')" onmouseout="UnTip()"><i class="fa fa-refresh"></i></button>
								
								<a href="<?php echo $_SERVER['PHP_SELF'] . '?action=add_to_wishlist&products_id='.$products[$i]['id'].'&qty='.$products[$i]['quantity'];?>" onmouseover="Tip('Move this item from your Shopping Cart to your Wish List')" onmouseout="UnTip()">
								    <input type="button" class="btn btn-info btn-sm" border="0" style="background-color:#777;"  alt="Remove from Shopping Cart and Add to Wish List" title=" Remove from Shopping Cart and Add to Wish List " value="Move to Wishlist"/>
							</a>
							<a href="<?php echo $_SERVER['PHP_SELF'] . '?action=remove_product&products_id='.$products[$i]['id'].'&qty='.$products[$i]['quantity'];?>" onmouseover="Tip('Remove item')" onmouseout="UnTip()">
...
?>
<?php } ?>
<td class=" text-center"><strong><?php echo SUB_TITLE_SUB_TOTAL.' '. $currencies->format($cart->show_total()); ?></strong></td>
...
if ($cc_id != '') {
       ...
       $coupon_less_amount = ': -'.$currencies->format($amount_array['coupon_amount']);
      	...
      	?>
      <div>
        <div align="right" class="main"><b>Discount Coupon: <?php echo $cc_coupon_amount.$coupon_less_amount; ?></b></div>
      </div>
    <?php }?>
    <?php
...
include(DIR_WS_MODULES . 'coupon_help_tip.php');
        ...
        Enter Coupon Code <input type="text" name="gv_redeem_code" style="width:100px;"> &nbsp; 
            <input type="submit" class="btn btn-info btn-sm" alt="IMAGE_REDEEM_VOUCHER" value="Apply" title=" IMAGE_REDEEM_VOUCHER ">
            </form>
...
<tfoot>
    <tr>
        ...
        <td><a onclick="show_loader()" class="btn btn-success btn-block">Checkout <i class="fa fa-angle-right"></i></a></td>
    </tr>
</tfoot>
</table>
</form>
