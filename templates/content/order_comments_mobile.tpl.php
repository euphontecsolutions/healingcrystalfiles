...
<?php
	...
	if ($check_array ['total'] == '0') {
		...
		echo '<div><div class="pageHeading" align="left" colspan="2">Thank you for your order# <a href="' . tep_href_link ( 'account_history_info.php', 'order_id=' . $HTTP_GET_VARS ['order_id'] ) . '" class="pageHeading">' . $HTTP_GET_VARS ['order_id'] . '</a> on ' . tep_date_short ( $order->info ['date_purchased'] ) . '</div></div>';
		echo '<div><div colspan="2"></div></div>';

		?>
				<div>
		<div class="mainCO">We really care about what you think. To help us
			...
			echo '<div>
							  	<div class="mainCO">
									<div border="0" width="100%" cellpadding="0" cellspacing="0">
										...
										<div><div></div></div>
											<div class="row">
											<div class="mainCO" valign="top" colspan="2">Comments:<br/>' . tep_draw_textarea_field ( 'enquiry[' . $i . ']', 'soft', 75, 6, '', 'id="enquiry' . $i . '" ' ) . tep_draw_hidden_field ( 'products_id[' . $i . ']', $order->products [$i] ['id'] ) . '</div>
										</div>
										</div></div>
