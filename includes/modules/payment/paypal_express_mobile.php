...
// When changing the shipping address due to no shipping rates being available, head straight to the checkout confirmation page
    ...
    function update_status() {
      global $order;
      if($order->billing['country_id'] == '188' || $order->delivery['country_id'] == '188'){
          $this->enabled = false;
      }elseif ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAYPAL_EXPRESS_ZONE > 0) ) {
        $check_flag = false;
    ...
    function checkout_initialization_method() {
      ...
      $string = '<a href="' . tep_href_link('ext/modules/payment/paypal/express_mobile.php', '', 'SSL') . '"><img src="' . $image_button . '" border="0" alt="" title="' . $button_title . '" /></a>';
    ...
    function pre_confirmation_check() {
      ...
      if (!tep_session_is_registered('ppe_token')) {
        tep_redirect(tep_href_link('ext/modules/payment/paypal/express_mobile.php', '', 'SSL'));
      }
    ...
    function after_process() {
      ...
      if(empty($response_array['PAYMENTINFO_0_TRANSACTIONID'])){
            $pp_result = 'No transaction ID found, Order Declined';
            $order_status = '100003';
      }
      if (stripos($response_array['PAYMENTINFO_0_PAYMENTSTATUS'], 'Pending') !== false &&  stripos($response_array['PAYMENTINFO_0_PAYMENTSTATUS'], 'echeck') !== false) {
            $order_status = '100004';
      }
      ...
      tep_db_query("update orders set payment_info = 'PayPal', orders_status = '" . $order_status . "' where orders_id = '" . (int)$insert_id . "'");
...
// format prices without currency formatting
    ...
    function setExpressCheckout($parameters) {
      ...
      $params = array('VERSION' => $this->api_version,
                      ...
                      'RETURNURL' => tep_href_link('ext/modules/payment/paypal/express_mobile.php', 'osC_Action=retrieve', 'SSL', true, false),
                      'CANCELURL' => tep_href_link('ext/modules/payment/paypal/express_mobile.php', 'osC_Action=cancel', 'SSL', true, false),
