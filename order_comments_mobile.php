...
if (!tep_session_is_registered('customer_id')) {
	$navigation->set_snapshot(array('mode' => 'SSL', 'page' =>  'order_comments.php?order_id='.$HTTP_GET_VARS['order_id']));
	...
	for($i=0;$i<sizeof($order->products);$i++){
		if($HTTP_POST_VARS['enquiry'][$i]!=''){
			if($HTTP_POST_VARS['star1'][$i]<'1'){
				...
				tep_redirect(tep_href_link('order_comments.php', 'order_id='.$HTTP_GET_VARS['order_id']));
			}
		}				
	}
